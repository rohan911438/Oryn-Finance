...
export function useContracts() {
  ...
  const createMarket = useCallback(
    async (marketData: {
      ...
      return handleContractCall(
        () => contractService.createMarket(marketData, signTransaction),
        'Market created successfully!',
        'Failed to create market'
      );
    },
    [signTransaction, handleContractCall]
  );
  ...
  const buyTokens = useCallback(
    async (
      ...
      return handleContractCall(
        () => contractService.buyTokens(marketContract, tokenType, amount, price, signTransaction),
  ...
  const sellTokens = useCallback(
    async (
      ...
      return handleContractCall(
        () => contractService.sellTokens(marketContract, tokenType, amount, price, signTransaction),
  ...
  const claimWinnings = useCallback(
    async (marketContract: string) => {
      return handleContractCall(
        () => contractService.claimWinnings(marketContract, signTransaction),
  ...
  const swapTokens = useCallback(
    async (
      ...
      return handleContractCall(
        () => contractService.swapTokens(fromToken, toToken, amountIn, minAmountOut, signTransaction),
  ...
  const addLiquidity = useCallback(
    async (
      ...
      return handleContractCall(
        () => contractService.addLiquidity(tokenA, tokenB, amountA, amountB, signTransaction),
  ...
  const stakeTokens = useCallback(
    async (amount: number, lockPeriod: number) => {
      return handleContractCall(
        () => contractService.stakeTokens(amount, lockPeriod, signTransaction),
  ...
  const voteOnProposal = useCallback(
    async (proposalId: number, choice: 'YES' | 'NO' | 'ABSTAIN') => {
      return handleContractCall(
        () => contractService.voteOnProposal(proposalId, choice, signTransaction),
